...
class ProductController extends Controller
{
        ...
        if (!empty($request->product_image)) {
            foreach ($request->product_image as $key => $image) {
                if ($request->hasFile('product_image.' . $key)) {
                    $img = Auth::user()->id . '_' . uniqid() . '.' . $image->getClientOriginalExtension();
    ...
    public function getProductDetails(Request $request)
    {
        ...
        $productTypeArr = ProductType::where('status', '1')->orderBy('order', 'asc')->pluck('name', 'id')->toArray();
return $targetArr;
        $view = view('admin.product.showProductDetails', compact('targetArr', 'productTypeArr'))->render();
        return response()->json(['html' => $view]);
    }
}
